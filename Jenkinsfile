...
String lambda_function_name = "admin_api_add"
...
pipeline {
    ...
    stages {
        ...
        stage('Deploy lambda to development') {
            when {
                // Only deploy if job param has been ticked ("true")
                equals expected: "true", actual: DEPLOY
                // Don't deploy if this is the example lambda
            ...
            steps {
                sh(
                    """
                    aws s3 cp ${target_file} s3://mdtp-lambda-functions-development --acl=bucket-owner-full-control --only-show-errors
                    aws s3 cp ${env.JOB_BASE_NAME}.zip.base64sha256 s3://mdtp-lambda-functions-development --content-type text/plain --acl=bucket-owner-full-control --only-show-errors
                    SESSIONID=\$(date +"%s")
                    AWS_CREDENTIALS=\$(aws sts assume-role --role-arn arn:aws:iam::${account_info['development']['account_id']}:role/RoleJenkinsInfraBuild --role-session-name \$SESSIONID --query '[Credentials.AccessKeyId,Credentials.SecretAccessKey,Credentials.SessionToken]' --output text)
                    export AWS_ACCESS_KEY_ID=\$(echo \$AWS_CREDENTIALS | awk '{print \$1}')
                    export AWS_SECRET_ACCESS_KEY=\$(echo \$AWS_CREDENTIALS | awk '{print \$2}')
                    export AWS_SESSION_TOKEN=\$(echo \$AWS_CREDENTIALS | awk '{print \$3}')
                    aws lambda update-function-code --function-name ${lambda_function_name} --s3-bucket mdtp-lambda-functions-development --s3-key ${env.JOB_BASE_NAME}.zip
                    """
                )
            }
        }
